<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    <!-- 颜色显示小方块 -->
    <div class="colorBtn"
      v-bind:style="[showColor,colorBtnOps]"
      v-on:click="openStatus = !disabled"
      v-bind:class="{ disabled: disabled, 'transparent-tag': transparentTag}">
    </div>
    ...
    <!-- 颜色色盘 -->
    <div class="box" v-bind:class="{ open: openStatus }" v-bind:style="[boxPosition]">
      <div class="hd">
      ...
      <div class="bd">
        ...
        <div class="box-menu">
          <button type="button" class="transparent-btn" v-on:click="transparentFn('transparent')">Transparent</button>
          <button type="button" class="ok-btn" v-on:click="selectOK">OK</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    colorBtnOps(){
      return this.colorBtnOps;
    },
    // 显示颜色
    showColor () {
      if (this.color) {
        if (this.color != 'transparent') {
          this.transparentTag = false;
          return {
            backgroundColor: this.color,
          }
        } else {
          this.transparentTag = true;
        }
  ...
  methods: {
    ...
    //触发type=color的input，但是在IE浏览器有兼容性的问题
    triggerHtml5Color () {
...
.m-colorPicker{
  ...
  .transparent-tag {
    &:before {
      ...
      height: 50%;
      width: 50%;
      background-color: rgba(154, 154, 154, 0.38);
      display: inline-block;
      position: absolute;
    }
    &:after {
